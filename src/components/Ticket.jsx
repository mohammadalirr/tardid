...
const Ticket = ({
  ...
  useEffect(() => {
    if (titleRefC.current.id === "title-0" && ticketCount !== 1) {
      ...
      // iconRef.current.className = "fa-solid fa-ticket fa-fade"
    }

    if (titleRef.current.className === "title-0") {
    ...
    // if (numRef.current.id !== "person-number-0") {
  ...
  useEffect(() => {
    if (
      radioRef.current.id === `radio-${index}` &&
      // genderRef.current.value.length > 1 &&
      phoneA === true
    ) {
