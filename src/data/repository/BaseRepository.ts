...
import { LOCAL_STORAGE_ACCESS, LOCAL_STORAGE_REFRESH } from "~/config/env"
import { BASE_API_URL } from "~/core/constants/ConstBaseURL"


const Fetcher = new FetchApi(BASE_API_URL, LOCAL_STORAGE_ACCESS, LOCAL_STORAGE_REFRESH)
...
class BaseRepository {
   ...
   * @returns
   */
  public async postFormData<TData>(
    formData: TData,
  ): Promise<AxiosResponse<any, any>> {
    return this.api.post(`${this.endpoint}`, formData, {
      headers: {
  ...
  public async patch<TData>(
    ...
    return this.api.patch(`${this.endpoint}/${id}`, formData)
  }
